var ICPAuth = (() => {
  ...
  // src/auth-entry.js
  var auth_entry_exports = {};
  __export(auth_entry_exports, {
    Ed25519KeyIdentity: () => Ed25519KeyIdentity,
    HttpAgent: () => HttpAgent,
    Principal: () => Principal,
    createAnonymousAgent: () => createAnonymousAgent,
    createAuthenticatedAgent: () => createAuthenticatedAgent
  });
...
`;
  ...
  requestAndRetryQuery_fn = async function(args) {
    ...
    try {
      ...
      if (fetchResponse.status === HTTP_STATUS_OK) {
        const queryResponse = decode(uint8FromBufLike2(await fetchResponse.arrayBuffer()));
        response = {
          ...queryResponse,
          httpDetails: {
            ok: fetchResponse.ok,
  ...
  // src/auth-entry.js
  function createAnonymousAgent(host) {
    return new HttpAgent({ host });
  }
  function createAuthenticatedAgent(host, identity) {
    return new HttpAgent({ host, identity });
  }
  return __toCommonJS(auth_entry_exports);
})();
...
*/
//# sourceMappingURL=icp-auth.js.map
