...
class TestDocumentSearchTool:
    """Test execute_tool('document_search', ...) with mocked vector store."""

    def test_document_search_no_context(self):
        """Document search without principal_id fails gracefully."""
        success, result = execute_tool("document_search", {"query": "test"})

        assert success is False
        assert "principal_id" in result.lower() or "context" in result.lower()

    def test_document_search_empty_query(self):
    ...
    @patch("services.embeddings.embed_text")
    def test_document_search_finds_documents(self, mock_embed, mock_get_vs):
        """Document search returns matching documents."""
        mock_embed.return_value = np.zeros(384)

        mock_vs = MagicMock()
        mock_vs.search_documents.return_value = [
            {"content": "Contract clause about termination", "score": 0.85, "filename": "contract.pdf"},
            {"content": "Termination notice period is 30 days", "score": 0.72, "filename": "contract.pdf"},
        ]
    ...
    @patch("services.vector_store.get_vector_store")
    @patch("services.embeddings.embed_text")
    def test_document_search_falls_back_to_messages(self, mock_embed, mock_get_vs):
        """Falls back to message history when no documents found."""
        mock_embed.return_value = np.zeros(384)

        mock_vs = MagicMock()
        mock_vs.search_documents.return_value = []
        mock_vs.search_messages.return_value = [
            {"content": "I mentioned earlier about the meeting", "score": 0.65, "role": "user"},
        ]
        mock_get_vs.return_value = mock_vs
    ...
    @patch("services.vector_store.get_vector_store")
    @patch("services.embeddings.embed_text")
    def test_document_search_no_results(self, mock_embed, mock_get_vs):
        """No results from either source returns informative message."""
        mock_embed.return_value = np.zeros(384)
