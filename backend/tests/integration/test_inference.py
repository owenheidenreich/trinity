"""
Integration Tests — Real LLM Inference
==========================================
These tests hit a real LLM instance. They validate the full request pipeline:
  - Flask routing → request parsing → LLM API call → response formatting.

Skip conditions:
  - LLM not running → entire file skipped
...
class TestBasicGeneration:
    """Test the core /generate endpoint with real LLM."""

    def test_health_check(self, client, llm_available):
        """Health endpoint should return 200 and model info."""
        resp = client.get("/health")
        data = resp.get_json()
        assert resp.status_code in (200, 503)  # 503 if LLM still loading
        assert "status" in data
        assert "model" in data
        assert "provider_id" in data

    def test_simple_generate(self, client, llm_available, test_model):
        """Simple generate should return a response from LLM."""
    ...
    def test_empty_prompt_rejected(self, client, llm_available):
        """Empty prompt should return 400."""
        resp = client.post("/generate/simple", json={"prompt": ""})
        assert resp.status_code == 400

    def test_generate_with_options(self, client, llm_available, test_model):
...
class TestStreamingGeneration:
    ...
    def test_simple_stream(self, client, llm_available, test_model):
...
class TestEndpointValidation:
    ...
    def test_prompt_length_limit(self, client, llm_available):
    ...
    def test_origin_validation(self, client, llm_available):
        """Bad origins should be rejected on POST."""
        resp = client.post("/generate", json={"prompt": "test"},
                          headers={"Origin": "https://evil.com"})
        assert resp.status_code == 403

    def test_metrics_endpoint(self, client, llm_available):
