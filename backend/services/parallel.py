...
try:
    from middleware.observability import PARALLEL_EXECUTIONS, PARALLEL_LATENCY, PROMETHEUS_AVAILABLE
except ImportError:
...
class ParallelAgentPipeline:
    ...
    def __init__(self, ollama_host: str, model_name: str, timeout_seconds: float = 120.0):
        """
    ...
    def legacy_pipeline(self):
        ...
        if self._legacy_pipeline is None:
            ...
            self._legacy_pipeline = AgentPipeline(self.ollama_host, self.model_name)
        ...
        **kwargs,
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            legacy_future = executor.submit(
                self._execute_legacy, question, context_messages or [], user_memory, **kwargs
            )
            langgraph_future = executor.submit(
                self._execute_langgraph, question, complexity, **kwargs
            )
        ...
        if winner == "legacy":
            ...
            final_response = (
                langgraph_result.response if langgraph_result.success else legacy_result.response
            )
    ...
    def _execute_legacy(
        self, question: str, context_messages: List[Dict], user_memory: Dict, **kwargs
    ) -> PipelineResult:
        ...
        try:
            result = self.legacy_pipeline.process(
                question=question, context_messages=context_messages, user_memory=user_memory
            )
            ...
            return PipelineResult(
                ...
                success=True,
                metadata={"passes_used": result.passes_used, "complexity": result.complexity},
            )
        except Exception as e:
    ...
    def _execute_langgraph(self, question: str, complexity: str, **kwargs) -> PipelineResult:
        ...
        try:
            ...
            final_state = execute_graph(
                user_message=question, complexity=complexity, max_iterations=5
            )
            ...
            return PipelineResult(
                ...
                duration_seconds=time.time() - start,
                success=True,
                metadata={"iterations": final_state.get("iteration", 0), "complexity": complexity},
            )
        except Exception as e:
    ...
    def _vote(self, legacy: PipelineResult, langgraph: PipelineResult) -> tuple:
        """
        ...
        if legacy_score > langgraph_score * 1.1:  # 10% margin
            return (
                "legacy",
                confidence,
                f"Legacy scored {legacy_score:.1f} vs {langgraph_score:.1f}",
            )
        elif langgraph_score > legacy_score * 1.1:
            return (
                "langgraph",
                confidence,
                f"LangGraph scored {langgraph_score:.1f} vs {legacy_score:.1f}",
            )
        else:
...
def get_parallel_pipeline(ollama_host: str = None, model_name: str = None) -> ParallelAgentPipeline:
    """
    ...
    if _parallel_pipeline is None:
        ...
        _parallel_pipeline = ParallelAgentPipeline(ollama_host=ollama_host, model_name=model_name)
