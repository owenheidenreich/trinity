...
from config import VOTING_CANDIDATES, VOTING_MIN_COMPLEXITY, VOTING_TEMPERATURES
...
try:
    ...
    def record_voting(outcome, participants):
        pass
...
class VotingResult:
    ...
    def __init__(
        self, selected_answer: str, confidence: float, candidates: List[str], scores: List[float]
    ):
...
def generate_candidates_parallel(
    ...
    with ThreadPoolExecutor(max_workers=n_candidates) as executor:
        futures = {executor.submit(generate_one, i): i for i in range(n_candidates)}
...
def run_voting_pipeline(
    ...
    if not force_voting and not should_use_voting(complexity):
        ...
        try:
            ...
            return VotingResult(
                selected_answer=answer, confidence=1.0, candidates=[answer], scores=[1.0]
            )
        except Exception as e:
    ...
    candidates = generate_candidates_parallel(
        generate_fn=generate_fn, prompt=prompt, n_candidates=n_candidates
    )
    ...
    return VotingResult(
        selected_answer=best_answer, confidence=confidence, candidates=candidates, scores=scores
    )
